...
document.getElementById('symptomsForm').addEventListener('submit', function(e) {
    ...
    // Question 1: Zones
    const zones = parseInt(document.querySelector('input[name="zones"]:checked').value);
    score += zones;
    ...
    // Question 4: Symptoms (checkboxes)
    const symptoms = document.querySelectorAll('input[name="symptoms"]:checked');
    symptoms.forEach(symptom => {
        score += parseInt(symptom.value);
    });
    
    // Question 5: Arrival time
    const arrival = parseInt(document.querySelector('input[name="arrival"]:checked').value);
    score += arrival;
    
    // Determine severity and tips
